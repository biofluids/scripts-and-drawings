...
C Local Newton iteration to calculate theta, currently only positive growth is allowed
      do while (kstep .ge. 3 .and. local_res > tol
...
C
      parameter(pi = 3.1415926d0, phi = 0.7853982d0, rin0 = 1.5d-3,
     1 rout0=2.25d-3, tol=1.d-6)
      double precision r0, theta0, rout, rin, r, theta
C phi = 45 degrees, rin0 = 1.5 mm, rout0 = 2.25 mm
C Assuming dr/r0 = const, dtheta/theta = [-1,0], dz = 0
C (pi-phi)/pi*(rout0**2-rin0**2) = rout**2-rin**2
C (rout-rout0)/rout0 = (rin - rin0)/rin0
      if (kstep .eq. 1) then
          ...
          rout = rout0*sqrt((pi-phi)/pi)
          rin = rin0*sqrt((pi-phi)/pi)
          r = r0 + r0*(rin-rin0)/rin0 ! new r
